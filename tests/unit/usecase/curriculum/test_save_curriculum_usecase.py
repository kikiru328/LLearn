...
class TestSaveCurriculumUseCase:
    @pytest.mark.asyncio
    async def test_execute_saves_curriculum_successfully(
        self, 
        ...
        user_id = uuid4()
        request = SaveCurriculumUseCaseRequest(
            user_id=user_id,
            title="CS 기초 학습", 
            goal="컴퓨터 과학 기초 이해",
        ...
        assert isinstance(result, SaveCurriculumUseCaseResponse)
